...
public class RootElement extends MyTreeNode
{
	...
	public JPanel gibDiagramm()
	{
		...
		Map<Point, String> values = new HashMap<Point, String>()
		{
			{
				put((new Point(1, 3)), "Bereich A") ;
				put((new Point(2, 1)), "Bereich B") ;
				put((new Point(1, 2)), "Bereich C") ;
				put((new Point(3, 1)), "Bereich D") ;
				put((new Point(2, 3)), "Bereich E") ;
			}
		} ;
		NormalizedMatrixSeries matrix = new NormalizedMatrixSeries("Bereiche",
		                                                           5,
		                                                           5) ;
		for( Point key : values.keySet() )
		{
		...
		for( Point key : values.keySet() )
		{
			...
			double x = key.y ;
			double y = key.x ;
			a = new XYTextAnnotation(values.get(key), x, y) ;
